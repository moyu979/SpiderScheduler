...
class NetworkApi:
    ...
    def __init__(self):
        self.blueprint = Blueprint('network_api', __name__, url_prefix='/api/network')
    ...
    def _register_routes(self):
        ...
        def update_network_config():
            ...
            try:
                ...
                # 验证配置字典是否存在
                if 'network' not in ConfigManager.config_dict:
                    return jsonify({
                        'success': False,
                        'error': '配置字典 network 不存在'
                    }), 400
                ...
                for key, value in data.items():
                    try:
                        # 验证键是否存在
                        if key not in ConfigManager.config_dict['network']:
                        ...
                        logger.info(f"更新网络配置: {key} = {value}")
